...
class SentenceREDataset(data.Dataset):
    ...
    def __init__(self, text_path, rel_path, pic_path, N,K,Q,process,rel2id, tokenizer, kwargs):
        """
        Args:
        ...
        self.K=K
        self.name=process
        self.class2datas={key:[] for key in rel2id.keys()}
        ...
        for i1 in range(len(f_lines)):
            ...
            if len(line) > 0:
                dic1 = eval(line)
                self.class2datas[dic1["relation"]].append(dic1)
                self.data.append(dic1)
                self.classes.append(dic1["relation"])
    ...
    def __getitem__(self, index):
        target_classes = random.sample(self.classes, self.N)  # 每次采样N个class
        support_lst = []
        query_lst = []
        # query_label = []
        ...
        for i, class_name in enumerate(target_classes):
            #测试或验证阶段
            sample_numbers = len(self.class2datas[class_name])

            #有些类别没有测试样例
            # if sample_numbers==0:
            #     new_class=class_name
            #     while new_class in target_classes or len(self.class2datas[new_class])==0:
            #         new_class=np.random.choice(self.classes,1,False)[0]
            #     class_name=new_class

            sample_numbers = len(self.class2datas[class_name])
            samples = self.class2datas[class_name]
            # print()
            # print("sample_numbers:",sample_numbers)
            # print("class_name:",class_name)
            if self.name == 'test' or self.name == 'val':
                if self.K + self.Q <= sample_numbers:
                    indices = np.random.choice(list(range(sample_numbers)), self.K + self.Q, False)
                else:  # 样本数量不够K+Q
                    indices = []
                    query_pair = np.random.choice(list(range(sample_numbers)), self.Q, False)  # query_set取的Q个不同instance
                    remain_pair = [i for i in list(range(sample_numbers)) if i not in query_pair]
                    # print("remain_pair1:",remain_pair)
                    try:
                        if sample_numbers==self.Q:
                            support_pair=[query_pair[0]]*self.K
                        else:
                            # print("remain_pair2:",remain_pair)
                            support_pair = np.random.choice(remain_pair, self.K,True)  # 从剩下的instances 选择K个instance 作为support_set，里面可能有重复
                    except:
                        assert False, samples[query_pair[0]]
                ...
                for j in indices:
                    ...
                    res = [i] + seq + list_p + list_r

                    if count < self.K:
                        support_lst.append(res)
                    else:
                        query_lst.append(res)
    ...
    def collate_fn(data):
        batch_support_temp=[[],[],[],[],[],[],[]]
        batch_query_temp=[[],[],[],[],[],[],[]]
        support_lst, query_lst=zip(*data)

        for i in range(len(support_lst)):
            for j in range(len(support_lst[i])):
                # print(support_lst[i][j])
                for k in range(len(support_lst[i][j])):
                    # print(support_lst[i][j][k],end=" ")
                    batch_support_temp[k].append(support_lst[i][j][k])

        for i in range(len(query_lst)):
            for j in range(len(query_lst[i])):
                # print(support_lst[i][j])
                for k in range(len(query_lst[i][j])):
                    # print(support_lst[i][j][k],end=" ")
                    batch_query_temp[k].append(query_lst[i][j][k])
        # print(len(support_lst))
        #生成support
        # data=list(zip(*support_lst))
        # print("len(data)",len(data))
        batch_support=[]
        seqs = batch_support_temp[1:]
        for seq in seqs:
            # seq=torch.from_numpy(np.array(seq))
            # print("#######seq.shape:",seq.shape)
            batch_support.append(torch.cat(seq, 0))  # (B, L)

        #生成query
        labels = batch_query_temp[0]
        batch_labels = torch.tensor(labels).long()  # (B)
        # print(batch_labels)
        batch_query=[]
        seqs = batch_query_temp[1:]
        for seq in seqs:
            # seq=torch.from_numpy(np.array(seq))
            batch_query.append(torch.cat(seq, 0))  # (B, L)

        # batch_support=torch.from_numpy(np.array(batch_support))
        # batch_query=torch.from_numpy(np.array(batch_query))
        return [batch_support,batch_query,batch_labels]

    def eval(self, pred_result,labels, use_name=False):
        """
        Args:
...
def SentenceRELoader(process,N,K,Q,text_path, rel_path, pic_path, rel2id, tokenizer, batch_size,
